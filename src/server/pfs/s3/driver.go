...
func (d *MasterDriver) bucket(pc *client.APIClient, r *http.Request, name string) (*Bucket, error) {
	...
	if len(parts) == 3 {
		// branch.type.repo
		repo = client.NewSystemRepo(parts[2], parts[1])
		branch = parts[0]
	} else if len(parts) == 2 {
		// branch.repo (assume "user" type, requiring explicit specification of branch otherwise)
